...
class logThread(threading.Thread):
	def __init__(self, threadID, name, srv, shell, pathSrv1, pathSrv2, pathSrv3, lineas_inicio, tiempo_logs, file_name, log_name, lineas_MAX, file_name2):
		threading.Thread.__init__(self)
		self.threadID = threadID
		self.name = name
		self.srv = srv
		self.shell = shell
		self.pathSrv1 = pathSrv1
		self.pathSrv2 = pathSrv2
		self.pathSrv3 = pathSrv3
	...
	def run(self):
		print "Iniciando: " + self.name
		get_log_servidor(self.srv, self.shell, self.pathSrv1, self.pathSrv2, self.pathSrv3, self.lineas_inicio, self.tiempo_logs, self.file_name, self.log_name, self.lineas_MAX, self.file_name2)
		print "Saliendo: " + self.name
		
# Definimos la funcion que extraeran y generaran los logs
def get_log_servidor(srv, shell, pathSrv1, pathSrv2, pathSrv3, lineas_inicio, tiempo_logs, file_name, log_name, lineas_MAX, file_name2):
	count_log = 1
	ruta_destino = [''] * len(lineas_inicio)
	result_srv1 = [''] * len(lineas_inicio)
	...
	while 1:
		...
		try:
			for i,path in enumerate(pathSrv):
				result_srv[i] = shell[server_inde[i]].run(["sh", "-c", "wc -l " + path + " | cut -d' ' -f1"])
		except Exception as cadena:
			print "Error: " + format(cadena)
			break;
		for result in result_srv1:
			lineas_actuales.append( int(result.output.rstrip()) + 1 )
		for result in result_srv2:
			lineas_actuales.append( int(result.output.rstrip()) + 1 )
		for result in result_srv3:
			lineas_actuales.append( int(result.output.rstrip()) + 1 )
		
		# Ponemos como limite 2000 lineas a transferir
		for index,lineas_actuales in enumerate(lineas_actuales):
			lineas.append( int(lineas_actuales) - int(lineas_inicio[index]) )
		...
		for index,line in enumerate(lineas_actuales):
			if lineas[index] >= lineas_MAX:
				lineas_actuales[index] = int(lineas_inicio[index]) + lineas_MAX
			
		# Creamos el archivo en el servidor actual
		path.append(pathSrv1)
		path.append(pathSrv2)
		path.append(pathSrv3)
		for index,lineas_actuales in enumerate(lineas_actuales):
			command.append('awk "NR >= ' + str(lineas_inicio[index]) + ' && NR <=' + str(lineas_actuales[index]) + ' " ' + path[index] + ' > "' + ruta_destino[index] + '"')
		command = 
