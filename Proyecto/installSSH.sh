...
sed -i -e"s/ipApache =/ipApache = \x22$ip\x22/" ~/Proyecto/config.conf
echo
...
sed -i -e"s/usuarioWeb =/usuarioWeb = \x22$usuario\x22/" ~/Proyecto/config.conf
echo
...
sed -i -e"s/ipPostgres =/ipPostgres = \x22$ip\x22/" ~/Proyecto/config.conf
...
sed -i -e"s/usuarioBD =/usuarioBD = \x22$usuario\x22/" ~/Proyecto/config.conf
echo
echo -n "[+] NOTA: Probablemente pida alguna autorización, favor de decir yes a todas las preguntas y escribir la contraseña del servidor POSTGRES cuando lo solicite, si la contraseña es incorrecta, favor de cancelar el script y volverlo a ejecutar. Verifica la IP y el usuario"
echo
echo
...
sed -i -e"s/ipWaf =/ipWaf = \x22$ip\x22/"  ~/Proyecto/config.conf
...
#Guardando el usuario en el archivo de configuracion, por defecto en el archivo no tiene un valor la variable usuarioPostgres
sed -i -e"s/usuarioWaf =/usuarioWaf = \x22$usuario\x22/"  ~/Proyecto/config.conf
echo
